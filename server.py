...
async def handleMessages(websocket):
    ...
    try:
        async for message in websocket:
            ...
            print('Received:', json_message) # for logging
...
def changeStatus(msg_id, conf_id, sent=False, received=False):
    json_msg = {
        'type': 'JM03',
        'message_id': msg_id,
        ...
        for user, websocket in connections.items():
            if len(messages_to_send[user]) > 0:
                ...
                try:
                    ...
                    print('Sent:', json.dumps(msg_for_receiver))
            ...
            if len(messages_sent[user]) > 0:
                ...
                try:
                    print('Sent:', changeStatus(message_id, conf_id, sent=True))
                    await websocket.send(changeStatus(message_id, conf_id, sent=True))
            ...
            if len(messages_received[user]) > 0:
                ...
                try:
                    print('Sent:', changeStatus(message_id, conf_id, received=True))
                    await websocket.send(changeStatus(message_id, conf_id, received=True))
